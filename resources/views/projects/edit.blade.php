                    ...
                    <form method="post" action="{{ route('projects.update', ['project' => $project]) }}">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 mt-6 gap-6">
                            ...
                            <div>
                                ...
                                <x-text-input id="book_title" name="book_title" type="text" class="mt-1 block w-full" :value="old('book_title', $project->book_title)" />
                                <x-input-error class="mt-2" :messages="$errors->get('book_title')" />
                            </div>

                            <div class="md:col-span-2">
                                <x-input-label for="book_description" :value="__('Book Description')" />
                                <x-text-input id="book_description" name="book_description" type="text" class="mt-1 block w-full" :value="old('book_description', $project->book_description)" />
                                <x-input-error class="mt-2" :messages="$errors->get('book_description')" />
                            </div>

                            <div>
                                <x-input-label for="book_creator" :value="__('Book Author')" />
                                <x-text-input id="book_creator" name="book_creator" type="text" class="mt-1 block w-full" :value="old('book_creator', $project->book_creator)" />
                                <x-input-error class="mt-2" :messages="$errors->get('book_creator')" />
                            </div>

                            <div>
                                <x-input-label for="book_subject" :value="__('Book Tag')" />
                                <x-text-input id="book_subject" name="book_subject" type="text" class="mt-1 block w-full" :value="old('book_subject', $project->book_subject)" />
                                <x-input-error class="mt-2" :messages="$errors->get('book_subject')" />
                            </div>

                            <div>
                                <x-input-label for="book_publisher" :value="__('Book Publisher')" />
                                <x-text-input id="book_publisher" name="book_publisher" type="text" class="mt-1 block w-full" :value="old('book_publisher', $project->book_publisher)" />
                                <x-input-error class="mt-2" :messages="$errors->get('book_publisher')" />
                            </div>

                            <div>
                                <x-input-label for="book_language" :value="__('Book Language')" />
                                <x-select-input id="book_language" name="book_language" class="mt-1 block w-full" :value="old('book_language', $project->book_language)" :options="config('languages.all')" />
                                <x-input-error class="mt-2" :messages="$errors->get('book_language')" />
                            </div>

                            <div>
                                <x-input-label for="book_width" :value="__('Book Width')" />
                                <x-text-input id="book_width" name="book_width" type="number" min="0" class="mt-1 block w-full" :value="old('book_width', $project->book_width)" required />
                                <x-input-error class="mt-2" :messages="$errors->get('book_width')" />
                            </div>

                            <div>
                                <x-input-label for="book_height" :value="__('Book Height')" />
                                <x-text-input id="book_height" name="book_height" min="0" type="number" class="mt-1 block w-full" :value="old('book_height', $project->book_height)" required />
                                <x-input-error class="mt-2" :messages="$errors->get('book_height')" />
                            </div>

                            <div>
                                <div class="flex items-center mb-4">
                                    <input @checked(old('book_webtoon_style', $project->book_webtoon_style)) id="book_webtoon_style" name="book_webtoon_style" type="checkbox" class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 focus:ring-2">

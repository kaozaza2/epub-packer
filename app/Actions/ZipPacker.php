...
class ZipPacker implements ZipPackerContract
{
    public function __construct(
        ...
        protected readonly CssMakerContract                $cssMaker,
    ...
    public function pack(string $title, array $images, array $options = []): string
    {
        ...
        $zip->addFromString('OEBPS/css/style.css', $this->cssMaker->make($options));
        ...
        collect($images)
            ...
            ->each(function ($page, $index) use ($resources, $options) {
                ...
                $resources->push([
                    'linear' => 'yes',
                    'idref' => 'page-' . ($index + 1),
                    'properties' => ($options['pager'] ?? 'no') === 'no'
                        ? 'rendition:page-spread-center'
                        : ($index % 2 === 0 ? 'rendition:page-spread-left' : 'rendition:page-spread-right'),
                    'tag' => 'spine',
                ]);
            })
        ...
        if (($options['pager'] ?? 'no') === 'no') {
            $resources->push(['tag' => 'metadata', 'tname' => 'meta', 'property' => 'rendition:orientation', 'text' => 'portrait']);
            ...
            $resources->push(['tag' => 'metadata', 'tname' => 'meta', 'property' => 'rendition:orientation', 'text' => 'landscape']);
            $resources->push(['tag' => 'metadata', 'tname' => 'meta', 'property' => 'rendition:spread', 'text' => 'both']);
        }

        $resources->push(['tag' => 'manifest', 'id' => 'css', 'href' => 'css/style.css', 'media-type' => 'text/css']);
